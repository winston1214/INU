...
if __name__ == '__main__':
    ...
    def compute_loss(x, y, theta):
        loss = -y * np.log(compute_h(x,theta)) + (1-y) * np.log(compute_h(x,theta)) # 201600779 김영민
        final_loss = (1/len(x)) * np.sum(loss)# 201600779 김영민
        return final_loss# 201600779 김영민

    def compute_grad(x, y, theta):
        gradient = (1/len(x)) * np.dot(x.T,compute_h(x,theta) - y)# 201600779 김영민
